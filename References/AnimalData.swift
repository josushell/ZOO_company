...
class TurtleType: AnimalType {
    override init() {
        ...
        goodDept = Hedgehog()
        badDept = RedPanda()
        explain = "극도의 효율을 추구하는 경향이 있으며 항상 도전적인 자세와 열정을 가지고 업무를 함. 전체적으로 본인 업무에 대한 이해도가 높고 충실하게 수행하여 타 사원들에게 모범이 됨. 하지만 평소 잦은 야근으로 인한 거북목 증후군이 상당히 진행된 것으로 보아 휴가 지급이 요망됨. 따라서  \(userName)씨는 개발팀으로 배치 바람."
    }
}
class KoalaType: AnimalType {
    override init() {
        ...
        goodDept = Gazelle()
        badDept = Turtle()
        explain = "예술감각이 뛰어나며 인내심과 창의력을 가지고 있음. 이를 바탕으로 업무 수행에서 긍정적인 결과를 보여줌. 어떤 일에도 굴하지 않는 강한 정신력을 보유하고 있음. 하지만 반복되는 야간 근무로 카페인 중독 증세를 보이며, 스트레스가 축적되어 심신 안정이 필요해보임. 따라서 \(userName)씨는 디자인팀으로 배치 바람."
    }
}
class RedPandaType: AnimalType {
    override init() {
        ...
        goodDept = Koala()
        badDept = Turtle()
        explain = "타고난 말솜씨를 보유하고 있으며, 평소 동료들과 원만한 인간관계를 유지하여 평가가 좋음. 항상 열정적인 자세를 가지고 있으며 이를 바탕으로 주어진 일을 성공적으로 해냄. 또한 강인한 정신력을 보유하고 있어 어떤 일에도 자신의 길을 개척하는 도전적인 태도를 가지고 있음. 따라서 \(userName)씨는 영업팀으로 배치 바람."
    }
}
