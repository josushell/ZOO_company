...
class ChoiceData_Ch2_part2 : ChoiceData {
    override init() {
        ...
        choice2 = ["그냥 웃어넘긴다", "현실적인 조언을 주는 뱁새"]
        choice3 = ["조용히 알파카 옆에 마스크를 놔준다", "밥을 꼭 같이 먹어야 하나? 혼자서 먹는다"]
    }
}
...
class Response_Ch2_part2: Response {
    ...
    override init() {
        ...
        player_response = [["알파카 대리님 장난하세요? 저 지금 침으로 샤워하겠어요", "알파카 대리님 완전 미스트가 따로없네요! 하하!", "(슬쩍)"],
                               ["좋아요 펭귄님 같이 먹어요~ ", "좋아요 뱁새님 같이 먹어요~", "아니에요 전 그냥 혼자 먹을게요~\n이 근처에 혼밥 가능한 맛집이.. 어디보자"]]
